...
import { getTodos, deleteTodos, completeTodos } from "../asyncTodoSlice";
import { API_URL } from "@/config/api";
import { Button } from "@/components/ui/button";
import AddTask from "@/components/add-task/add-task";
import EditTask from "@/components/edit-task/edit-task";
import GetTask from "@/components/get-task/get-task";
import ImagesPopover from "@/components/images-popover/images-popover";
              ...
              <tr key={e.id} className="even:bg-muted/30 hover:bg-muted/40 transition-colors">
                ...
                <td className="px-4 py-3 border-t border-border">
                  <div className="flex justify-center gap-2 flex-wrap">
                    <Button className="cursor-pointer" variant="secondary" size="sm" onClick={() => dispatch(completeTodos(e.id))}>
                    ...
                    <Button className="cursor-pointer" variant="secondary" size="sm" onClick={() => dispatch(deleteTodos(e.id))}>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
          ...
          <div key={e.id} className="rounded-xl border border-border bg-muted/30 p-4 shadow-sm space-y-4">
            ...
            <div className="pt-1">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="ghost" size="icon" onClick={() => dispatch(completeTodos(e.id))}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                    <path fillRule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 12.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clipRule="evenodd" />
                  </svg>
                </Button>
                ...
                <Button variant="ghost" size="icon" onClick={() => dispatch(deleteTodos(e.id))}>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
                    <path d="M3.53 2.47a.75.75 0 0 0-1.06 1.06l18 18a.75.75 0 1 0 1.06-1.06l-18-18ZM20.25 5.507v11.561L5.853 2.671c.15-.043.306-.075.467-.094a49.255 49.255 0 0 1 11.36 0c1.497.174 2.57 1.46 2.57 2.93ZM3.75 21V6.932l14.063 14.063L12 18.088l-7.165 3.583A.75.75 0 0 1 3.75 21Z" />
                  </svg>
                </Button>
