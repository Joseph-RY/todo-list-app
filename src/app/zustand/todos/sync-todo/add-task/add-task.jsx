...
export default function AddTask() {
  ...
  const data = useSyncStore((state) => state.data);
  const addTodos = useSyncStore((state) => state.addTodos);
  ...
  const handleAdd = (e) => {
    ...
    const newId = data.length > 0 ? Math.max(...data.map((t) => t.id)) + 1 : 3001;
    const newImageIdBase = data.length > 0 ? Math.max(...data.flatMap((t) => t.images.map((i) => i.id))) + 1 : 5001;

    const imagesWithId = imageLinks.map((img, idx) => ({
      id: newImageIdBase + idx,
